...
const Page = () => {
  ...
  useEffect(() => {
    const fetchCollege = async () => {
      try {
        setLoading(true);
        const res = await apiClient.user.colleges.getBySlug(slug as string);
        if (res) setCollege(res);
      } catch (err) {
        ...
        <Card className="pt-0 overflow-hidden col-span-2">
          <CardHeader className="p-0">
            {loading ? (
              ...
              <div className="h-full w-full aspect-square bg-muted flex items-center justify-center text-muted-foreground">
                No image available
              </div>
            )}
          </CardHeader>
          <CardTitle className="px-5 md:text-4xl text-2xl truncate text-primary font-semibold">
            {loading ? <Skeleton className="h-8 w-2/3" /> : college?.name || ""}
          </CardTitle>
